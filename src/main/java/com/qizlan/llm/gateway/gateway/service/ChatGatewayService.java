...
public class ChatGatewayService {
    ...
    private Mono<ChatCompletionResponse> tryComplete(RequestExecutionState state, int index) {
        if (index >= state.candidates().size()) {
            requestLogService.logGatewayFailure(state.path(), state.request().model(), state.apiKey(), gatewayStatus(state.lastError()), System.currentTimeMillis() - state.startedAt(), state.attempts(), state.request(), state.requestContext());
        ...
        return providerGateway.completeAsync(state.request(), mapping.getProvider().getId(), mapping.getModelName())
                ...
                .map(result -> {
                    ...
                    requestLogService.logGatewayRequest(state.path(), state.request().model(), result.providerId(), state.apiKey(), response.usage(), 200, System.currentTimeMillis() - state.startedAt(), state.attempts(), state.request(), response, state.requestContext());
                    return response;
                })
    ...
    private Mono<ChatCompletionResponse> handleCompleteProviderFailure(RequestExecutionState state, int index, ModelProviderMappingEntity mapping, UpstreamProviderException ex) {
        ...
        state.lastError(ex);
        if (!ex.isRetryable()) {
            requestLogService.logGatewayFailure(state.path(), state.request().model(), state.apiKey(), ex.getGatewayStatus(), System.currentTimeMillis() - state.startedAt(), state.attempts(), state.request(), state.requestContext());
    ...
    private Flux<ProviderStreamEvent> tryStream(RequestExecutionState state, ProviderStreamFormat format, int index) {
        ...
        return providerGateway.streamChatAsync(state.request(), mapping.getProvider().getId(), mapping.getModelName(), format)
                ...
                .doOnComplete(() -> {
                    ...
                    state.attempts().add(new RoutingAttempt(mapping.getProvider().getId(), mapping.getModelName(), 200, "none", true));
                    requestLogService.logStreamRequest(state.path(), state.request().model(), mapping.getProvider().getId(), state.apiKey(), 200, System.currentTimeMillis() - state.startedAt(), firstTokenAt.get() < 0 ? 0 : firstTokenAt.get() - state.startedAt(), state.attempts(), state.request(), state.requestContext());
                })
