...
class RoutingIntegrationTest extends BaseGatewayTest {
    ...
    @Autowired
    private RoutingProjectionStateService routingProjectionStateService;
    ...
    @Test
    void routingCacheRefreshesAfterProjectionVersionChange() {
        mockProviderAdapter.enqueueCompletionResponse("openai", mockResponse("openai", "model-5", "OpenAI first", 7, 3, 10));

        webTestClient.post().uri("/v1/chat/completions")
                .header("Authorization", "Bearer test-api-key")
                .contentType(org.springframework.http.MediaType.APPLICATION_JSON)
                .bodyValue("""
                        {"model":"gateway-text","messages":[{"role":"user","content":"first route"}]}
                        """)
                .exchange()
                .expectStatus().isOk()
                .expectBody()
                .jsonPath("$.metadata.used_provider").isEqualTo("openai");

        ModelEntity gatewayText = modelRepository.findById("gateway-text")
                .orElseThrow(() -> new IllegalStateException("Missing gateway-text model"));
        ProviderEntity anthropic = providerRepository.findById("anthropic")
                .orElseThrow(() -> new IllegalStateException("Missing anthropic provider"));
        ensureMapping(gatewayText, anthropic, "claude-3-5-sonnet", 5);
        routingProjectionStateService.markRoutingMetadataChanged("test-priority-update");

        mockProviderAdapter.enqueueCompletionResponse("anthropic", mockResponse("anthropic", "model-6", "Anthropic after projection refresh", 8, 4, 12));

        webTestClient.post().uri("/v1/chat/completions")
                .header("Authorization", "Bearer test-api-key")
                .contentType(org.springframework.http.MediaType.APPLICATION_JSON)
                .bodyValue("""
                        {"model":"gateway-text","messages":[{"role":"user","content":"second route"}]}
                        """)
                .exchange()
                .expectStatus().isOk()
                .expectBody()
                .jsonPath("$.choices[0].message.content").isEqualTo("Anthropic after projection refresh")
                .jsonPath("$.metadata.used_provider").isEqualTo("anthropic");
    }
}
