...
public class ImageGatewayService {
    ...
    public ImageGatewayService(RoutingService routingService, ProviderGateway providerGateway, RequestLogWriteService requestLogWriteService, ProviderHealthService providerHealthService, IamRuleService iamRuleService, GuardrailService guardrailService) {
    ...
    private Mono<ImageDtos.ImageResponse> tryGenerate(ImageExecutionState state, ImageDtos.ImageGenerationRequest request, int index) {
        if (index >= state.candidates().size()) {
            requestLogWriteService.logGatewayFailure(state.path(), state.requestedModel(), state.apiKey(), 502, System.currentTimeMillis() - state.startedAt(), state.attempts(), request, state.requestContext());
        ...
        return providerGateway.generateImageAsync(request, mapping.getProvider().getId(), mapping.getModelName())
                .doOnNext(response -> {
                    ...
                    requestLogWriteService.logImageRequest(state.path(), state.requestedModel(), mapping.getProvider().getId(), state.apiKey(), 200, System.currentTimeMillis() - state.startedAt(), state.attempts(), request, response, state.requestContext());
                })
    ...
    private Mono<ImageDtos.ImageResponse> handleImageFailure(ImageExecutionState state, int index, ModelProviderMappingEntity mapping, UpstreamProviderException ex, java.util.function.Supplier<Mono<ImageDtos.ImageResponse>> next) {
        ...
        if (!ex.isRetryable()) {
            requestLogWriteService.logGatewayFailure(state.path(), state.requestedModel(), state.apiKey(), ex.getGatewayStatus(), System.currentTimeMillis() - state.startedAt(), state.attempts(), state.requestedModel(), state.requestContext());
